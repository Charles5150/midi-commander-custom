...
| 5 | Media keys |
| 6 | Tap tempo, clock start/stop, transport, BPM up/down (hold SYNC for 120 BPM), and TREM, a tremolo on CC 14 that follows the tempo |
...
| 12–30 | A setlist: each bank selects its patch on entry and has looper controls. On song 1 (bank 12), D is PG 2, which shows bank 31 as its second page |
...
|---|---|---|---|---|---|---|
| `CommandType` | | | | | | `PC`, `PCInc`, `CC`, `CCInc`, `Note`, `PB`, `Key`, `Media`, `Bank`, `SysEx`, `Tap`, `Start`, `Stop`, `Panic`, `Scene`, `Wait`, `Ramp`, `LFO`, `Exp`, `Cycle` (short press only), or empty for none |
...
| `OnValue_(CC/PB)` | | ✓ | | ✓ | ✓ | CC: value on press (0–127). PB: −8192..8191. Key: key name. Media: media key name. Cycle: the state's label, up to 4 characters. Exp: the pedal, 1 or 2. LFO: the length of a cycle, `1/16T` `1/16` `1/8T` `1/8` `1/4T` `1/8.` `1/4` `1/2T` `1/4.` `1/2` `1/2.` `1/1` `2/1` `4/1` (empty for `1/4`) |
...
| `KeyMode_(Key)` | | | | | ✓ | Normal / Down / Up. CCInc and PCInc: Up / Down / Up Repeat / Down Repeat. Tap: Tap / Clock / Set / Up / Down / Up Repeat / Down Repeat. Exp: CC / Off / Own. LFO: Sine / Triangle / SawUp / SawDown / Square / Random (empty for Sine) |
