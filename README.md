...
- **A banner at power on.** The configuration's name and the firmware version can cross the display in large letters as the pedal starts, so you see at a glance which configuration came up. Any switch cuts it short, and the pedal answers from the first press. Turn it on with `Boot_Banner`.
