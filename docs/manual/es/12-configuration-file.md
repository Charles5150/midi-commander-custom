...
Un archivo escrito a mano se empaqueta tal como está escrito o dice qué está mal y dónde. `CommandType` se lee sin importar mayúsculas, así que `cc` es `CC`, y una columna de comando que falte en el archivo cuenta como vacía. Lo que la pedalera no puede hacer detiene el empaquetado con el sitio y el motivo, como `Button_Settings bank 2 button 3, command D (CC): Channel must be 1-16, not '17'`: un `CommandType` desconocido, un canal fuera de 1-16, un Pitch Bend fuera de -8192 a 8191, un `OffValue` de CC fuera de 0-127 y distinto de 128, un número de `SysEx` por encima de 15, o un ajuste global que no cabe. El configurador comprueba lo mismo al pulsar **Save CSV**, y no graba nada hasta que se corrige; `CSV_to_Flash.py` lo muestra y sale con el código 2.
...
| `Link_Toggles` | Link toggles | Y / N | Cada vez que un botón toggle envía un `CC` o una `Note`, todos los demás botones toggle que envían ese mismo CC o nota por el mismo canal, en cualquier banco, toman el estado que dice ese mensaje, con las reglas de `LED_Feedback`: su LED, su casilla en la pantalla y lo que enviará su próxima pulsación. Los botones se enlazan por lo que envían, no hay que marcar nada. No se envía nada más, así que no pueden dispararse entre sí; una lista con un `Listen` va por lo que dice el aparato. Ver [Toggles enlazados](05-buttons.md#toggles-enlazados). Por defecto N. Necesita el firmware 0.77 (bit 1 del byte global 35, junto a `LED_Feedback` en el bit 0). |
