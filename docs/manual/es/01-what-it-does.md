...
- **Reposo por inactividad.** Tras los minutos que elijas sin que nadie la toque, la pantalla y los LEDs se apagan. Cualquier pisada o movimiento de un pedal de expresión los vuelve a encender, y la pisada que la despierta sigue haciendo su trabajo, así que en el escenario no se pierde nada. Importa con baterías, donde no hay ordenador que suspenda el bus USB.
- **Modo reposo.** Cuando el ordenador al que está conectada se suspende, los LEDs y la pantalla se apagan; vuelven cuando se despierta.

## Con un ordenador y otros equipos

*En detalle: [Plantillas y equipos](11-devices.md).*

- **LEDs que siguen al ordenador.** Con `LED_Feedback` activado, un CC o una nota que llega por USB enciende o apaga los botones toggle que lo envían, así que la pedalera enseña lo que de verdad está encendido cuando cambias un efecto desde una DAW o desde el editor del ampli. A un equipo que informa en otro CC distinto del que recibe, o con otros valores, se le sigue con un comando `Listen` en el botón.
