...
A configuration is a CSV with several sections, each introduced by a line starting with `*` and the section name. Lines starting with `#` are comments; a `#` anywhere else is part of the data, so a bank can be called `F#m`. The configurator reads and writes this format, and you can also edit it in a spreadsheet.
...
| `Link_Toggles` | Link toggles | Y / N | Whenever a toggle button sends a `CC` or a `Note`, every other toggle button sending the same CC or note on the same channel, in any bank, takes the state that message describes, by the rules of `LED_Feedback`: its LED, its display cell and what its next press sends. Buttons are linked by what they send, nothing to mark. Nothing more is sent, so they cannot set each other off; a list with a `Listen` goes by the device instead. See [Linked toggles](05-buttons.md#linked-toggles). Default N. Needs firmware 0.77 (bit 1 of global byte 35, beside `LED_Feedback` in bit 0). |
