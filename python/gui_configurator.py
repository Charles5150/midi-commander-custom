...
class MidiCommanderGUI(ctk.CTk):
    def __init__(self):
        ...
        self.logo_label = ctk.CTkLabel(self.sidebar_frame, text="Midi Commander\nConfigurator", font=ctk.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.btn_load = ctk.CTkButton(self.sidebar_frame, text="Load CSV", command=self.load_csv)
        self.btn_load.grid(row=1, column=0, padx=20, pady=10)

        self.btn_save = ctk.CTkButton(self.sidebar_frame, text="Save CSV", command=self.save_csv)
        self.btn_save.grid(row=2, column=0, padx=20, pady=10)
        
        self.btn_flash = ctk.CTkButton(self.sidebar_frame, text="FLASH TO DEVICE", fg_color="red", hover_color="darkred", command=self.flash_device)
    ...
    def setup_button_tab(self):
        ...
        self.bank_selector = ctk.CTkOptionMenu(self.top_bar, command=self.on_bank_change)
        ...
        self.lbl_editing = ctk.CTkLabel(self.cmd_editor_frame, text="Select a button to edit", font=("Arial", 16))
    ...
    def parse_csv_sections(self, filepath):
        ...
        for i, tline in enumerate(title_lines):
            ...
            try:
                ...
                df = pd.read_csv(io.StringIO(csv_data), delimiter=",", header=0, engine='python', dtype=str).dropna(how="all")
    ...
    def on_bank_change(self, bank_val):
        ...
        rows = self.df_buttons[self.df_buttons['Bank_Number'].astype(str) == str(bank_val)]
        ...
        for i, row_data in rows.iterrows():
            ...
            btn = ctk.CTkButton(self.button_matrix_frame, text=f"Button {btn_id}", 
    ...
    def load_button_commands(self, row_index, btn_id):
        ...
        self.lbl_editing = ctk.CTkLabel(self.cmd_editor_frame, text=f"Editing: Bank {self.bank_selector.get()} - Button {btn_id}", font=("Arial", 16, "bold"))
        ...
        # --- Headers ---
        headers = ["Slot", "Type", "Channel", "Number", "On Value", "Off Value", "Toggle"]
        widths = [40, 80, 60, 60, 70, 70, 60]
        ...
        for i, slot in enumerate(slots):
            ...
            # Slot Label
            ctk.CTkLabel(table_frame, text=slot, width=widths[0]).grid(row=row, column=0, padx=5, pady=5)
            
            # Type
            type_val = current_row.get(f'{slot}_CommandType', '')
            if pd.isna(type_val): type_val = ""
            combo_type = ctk.CTkComboBox(table_frame, values=["", "CC", "PC", "Note", "PB"], width=widths[1])
            ...
            if pd.notna(ch_val): ent_ch.insert(0, str(ch_val).replace('.0', '')) # Simple cleanup
            ...
            if pd.notna(num_val): ent_num.insert(0, str(num_val).replace('.0', ''))
            ...
            if pd.notna(on_val): ent_on.insert(0, str(on_val).replace('.0', ''))
            ...
            if pd.notna(off_val): ent_off.insert(0, str(off_val).replace('.0', ''))
            ...
            if str(tog_val).strip().upper() == 'Y':
                chk_tog.select()
            chk_tog.grid(row=row, column=6, padx=5, pady=5)
    ...
    def apply_button_changes(self):
        ...
        for w in self.cmd_widgets:
            ...
            self.df_buttons.at[idx, f'{s}_Channel_(PC/CC/Note/PB)'] = safe_val(w['ch'].get())
            self.df_buttons.at[idx, f'{s}_Number_(PC/CC/Note)'] = safe_val(w['num'].get())
    ...
    def flash_device(self):
        ...
        # Ensure user saved
        ans = messagebox.askyesno("Flash Device", "Are you ready to flash? Ensure the Midi Commander is connected via USB.\n(This will take a few seconds)")
        if not ans: return

        # Use subprocess to isolate execution and prevent app crashes
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "CSV_to_Flash.py")
        ...
        try:
            ...
            if p.returncode == 0:
                ...
                if "no input found" in p.stdout.lower() or "no outputs found" in p.stdout.lower():
